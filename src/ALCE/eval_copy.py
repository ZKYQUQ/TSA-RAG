...
QA_MODEL=os.getenv("ALCE_QA_MODEL", "deepset/roberta-large-squad2")
AUTOAIS_MODEL=os.getenv("ALCE_AUTOAIS_MODEL", "google/t5_xxl_true_nli_mixture")
...
def compute_mauve(data):
    ...
    out = mauve.compute_mauve(
        ...
        verbose=True,
        batch_size=8,
        featurize_model_name=os.getenv("MAUVE_FEATURIZE_MODEL", "gpt2-large")
    )
...
def compute_claims_with_individual_socres(data):
    global autoais_model, autoais_tokenizer
    if autoais_model is None:
        logger.info("Loading AutoAIS model...")
        # autoais_model = AutoModelForSeq2SeqLM.from_pretrained(AUTOAIS_MODEL, torch_dtype=torch.bfloat16, max_memory=get_max_memory(), device_map="auto")
        autoais_model = AutoModelForSeq2SeqLM.from_pretrained(AUTOAIS_MODEL, torch_dtype=torch.bfloat16, max_memory=get_max_memory(), device_map={"": "cuda:2"})
        autoais_tokenizer = AutoTokenizer.from_pretrained(AUTOAIS_MODEL, use_fast=False)
    ...
    for item in tqdm(data):
        ...
        for claim in claims:
            entail += _run_nli_autoais(normalized_output, claim)
        scores.append(entail / len(claims))
        individual_scores[item["id"]] = entail / len(claims)
    return 100 * np.mean(scores), individual_scores
...
def compute_autoais(data,
    ...
    for item in tqdm(data):
        ...
        for sent_id, sent in enumerate(sents):
            ...
            # Find references
            ref = [int(r[1:])-1 for r in re.findall(r"\[\p+", sent)] # In text citation id starts from 1
...
def save_compute_qampari_f1(data, cot=False):
    prec = []
    rec = []
    rec_top5 = []
    f1 = []
...
def compute_qampari_f1(data, cot=False):
    ...
    for item in data:
        loc_acc = []
        for short_answers in item['answers']:
            loc_acc.append(exact_presence(short_answers, item["output"]))
        acc.append(np.mean(loc_acc))
        hit.append( int(np.mean(loc_acc) == 1) )
        top5_acc.append(1.0 if sum(loc_acc) >= 5 else np.mean(loc_acc))
        top5_hit.append( int(sum(loc_acc) >= 5 or np.mean(loc_acc) == 1) )

    return {
        "str_em": 100 * np.mean(acc),
        "str_hit": 100 * np.mean(hit),
        "str_em_5": 100 * np.mean(top5_acc),
        "str_hit_5": 100 * np.mean(top5_hit)
    }


def compute_rouge_with_individual_scores(data):
    """Compute both aggregate and individual ROUGE scores"""
    ...
    for item in data:
        hyp = '\n'.join(sent_tokenize(item["output"].lower()))
        
        if "annotations" in item and item['annotations'] is not None:
            ref1 = '\n'.join(sent_tokenize(item["annotations"][0]["long_answer"].lower()))
            ref2 = '\n'.join(sent_tokenize(item["annotations"][1]["long_answer"].lower()))
            
            score1 = scorer.score(ref1, hyp)
            score2 = scorer.score(ref2, hyp)
            
            if score1['rougeLsum'].fmeasure > score2['rougeLsum'].fmeasure:
                best_score = score1
            else:
                best_score = score2
        else:
            ref = '\n'.join(sent_tokenize(item["answer"].lower()))
            best_score = scorer.score(ref, hyp)
        
        aggregator.add_scores(best_score)
        individual_scores[item["sample_id"]] = 100 * best_score['rougeLsum'].fmeasure
        # individual_scores.append(100 * best_score['rougeLsum'].fmeasure)
    
    aggregate_score = 100 * aggregator.aggregate()['rougeLsum'].mid.fmeasure
    
    return aggregate_score, individual_scores

def compute_str_em_with_individual_scores(data):
    """Compute STR-EM metric (only for ASQA)
    Args:
        data: requires field `qa_pairs/short_answers` and `output`
    Returns:
    ...
    for item in data:
        ...
        acc.append(np.mean(loc_acc))
        hit.append( int(np.mean(loc_acc) == 1) )
        acc_dict[item["sample_id"]] = np.mean(loc_acc)
        hit_dict[item["sample_id"]] = int(np.mean(loc_acc) == 1)

    return 100 * np.mean(acc), 100 * np.mean(hit), acc_dict, hit_dict
...
def main():
    ...
    for i in range(len(data)):
        ...
        data[i]['output'] = data[i]['output'].strip().replace("\n", " ")  # NOTE: 
    ...
    if qampari:
        ...
        result.update(save_compute_qampari_f1(normalized_data, cot=args.cot))
        save_normalized_data = copy.deepcopy(data)
        # 计算每个样本的详细指标
        prec = []
        rec = []
        rec_top5 = []
        ...
        for item in save_normalized_data:
            if args.cot:
                if ":" in item['output']:
            ...
            precision = sum([p in flat_answers for p in preds]) / len(preds) if len(preds) > 0 else 0
            recall = sum([any([x in preds for x in a]) for a in answers]) / len(answers)
            recall_top5 = min(5, sum([any([x in preds for x in a]) for a in answers])) / min(5, len(answers))
            ...
            if (precision + recall_top5) == 0:
                ...
                f1_top5_score = 2 * precision * recall_top5 / (precision + recall_top5)
        ...
        df.to_excel(args.f.split(".json")[0] + "_invidual_score.xlsx", index=False, engine='openpyxl')
    ...
    if args.claims_nli:
        ...
        result["claims_nli"], individual_socres = compute_claims_with_individual_socres(normalized_data)
    ...
    if "asqa" in args.f:
        ...
        save_normalized_data = copy.deepcopy(data)
        result['str_em'], result['str_hit'], acc_dict, hit_dict = compute_str_em_with_individual_scores(save_normalized_data)
        if not args.no_rouge:
            aggregate_score, individual_scores = compute_rouge_with_individual_scores(save_normalized_data)
        print(f"str_em: {result['str_em']}, str_hit: {result['str_hit']}")
        print(f"rougeLsum: {aggregate_score}")
        
        excel_data = []
        for item in save_normalized_data:
            new_item = {}
            new_item["sample_id"] = item["sample_id"]
            new_item["question"] = item["question"]
            new_item["short_answers"] = [qa_pair["short_answers"] for qa_pair in item["qa_pairs"]]
            new_item["answers"] = [annotation["long_answer"] for annotation in item["annotations"]]
            new_item["output"] = item["output"]
            
            if item["sample_id"] not in acc_dict:
                print(f">>>> {item['sample_id']} not in acc_dict")
            new_item["em_acc"] = acc_dict[item["sample_id"]]
            new_item["hit"] = hit_dict[item["sample_id"]]
            if item["sample_id"] not in individual_scores:
                print(f">>>> {item['sample_id']} not in individual_scores")
            new_item["rougeLsum"] = individual_scores[item["sample_id"]]

            if "pre_retrieved_passages" in item:
                new_item["passages"] = item["pre_retrieved_passages"]
            if "routing_passages" in item:
                new_item["routing_passages"] = item["routing_passages"]

            new_item["qa_pairs"] = item["qa_pairs"]
            new_item["annotations"] = item["annotations"]
            
            
            excel_data.append(new_item)
        ...
        for item in data:
            ...
            new_item["output"] = item["output"]
            new_item["claims"] = item["claims"]
            new_item["claims_score"] = individual_socres[item["id"]]
